defmodule PropertiesWeb.MapView do
  ...
  def render("index.json", %{shapefiles: shapefiles}) do
    covs =
      Enum.map(shapefiles, & &1.adjacent_cov)
      |> Enum.filter(&(&1 > 0.0))
      |> Enum.sort()
  ...
  def render("lead_index_reject.json", %{set: set, shapefiles: shapefiles}) do
    {set, shapefiles} =
      Enum.reduce(shapefiles, {set, []}, fn shapefile, {set, list} ->
  ...
  def render("lead_show.json", %{shapefile: shapefile}) do
    {color, address} =
      if not is_nil(shapefile.lead_service_line_address) do
    ...
    %{
      ...
      properties: %{
        ...
        style: %{
          weight: 2,
          color:
            if(shapefile.type in ["BIKE LANE", "BUFFERED BIKE LANE", "PROTECTED BIKE LANE"],
              do: "#4AA564",
              else: "#E31C3D"
            ),
          opacity: 1,
  ...
  defp assessment_sq_foot(%{lot_area: area, last_assessment_land: assessment})
       when area > 0 and assessment > 0 do
    assessment / area
  end
