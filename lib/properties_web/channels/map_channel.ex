defmodule PropertiesWeb.MapChannel do
  ...
  def handle_in("layer_change", %{"layer" => layer}, socket) do
    socket =
      assign(socket, :object_id_set, MapSet.new())
      |> assign(:layer, layer)
  ...
  def handle_in("location_change", params, %{assigns: %{layer: "bike_lanes"}} = socket) do
    shapefiles =
      fetch_bike_lanes(params)
      |> Enum.reject(fn shape ->
        MapSet.member?(socket.assigns.object_id_set, PropertiesWeb.MapView.id(shape))
      end)
  ...
  def handle_in("location_change", params, %{assigns: %{layer: "lead"}} = socket) do
    ...
    {set, response} =
      PropertiesWeb.MapView.render("lead_index_reject.json",
  ...
  def fetch_bike_lanes(params) do
    ...
    off_street_path_shapefiles =
      Properties.OffStreetPathShapeFile.list(x_min, y_min, x_max, y_max)
