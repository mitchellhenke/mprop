defmodule Properties.Twitter do
  ...
  def reply(tweet, license_plate) do
    ...
    license_plate =
      String.upcase(license_plate)
      |> String.replace(" ", "")
      |> String.replace("-", "")

    tickets =
      ParkingTicket.filter_by_license_plate(ParkingTicket, license_plate)
    ...
    tickets_by_year =
      Enum.group_by(tickets, fn ticket -> ticket.date.year end)
    ...
    with tweets <- Map.get(webhook_callback_body, "tweet_create_events", []),
         tweets <-
           Enum.filter(tweets, fn tweet ->
             Map.get(tweet, "retweeted") == false && get_in(tweet, ["user", "protected"]) == false &&
               get_in(tweet, ["user", "id"]) != 1_157_650_398_649_507_840
           end) do
  ...
  def authenticated_request(url, parameters) do
    ...
    req_params =
      Enum.into(req_params, %{})
      |> URI.encode_query()

    {_, _status, _headers, client} =
      :hackney.post(
        url,
